...
import ru.kode.android.app.quality.plugin.core.messages.requiredConfigurationNotFoundMessage
...
@Suppress("TooGenericExceptionCaught") // Need to catch all exceptions
inline fun <reified T : Any> AppQualityDomainObjectContainer<T>.getByNameOrRequiredCommon(
    name: String,
    defaultName: String = COMMON_CONTAINER_NAME,
): T {
    return try {
...
inline fun <reified T : Any> AppQualityDomainObjectContainer<T>.getByNameOrNullableCommon(
    name: String,
    defaultName: String = COMMON_CONTAINER_NAME,
): T? {
    return findByName(name) ?: findByName(defaultName)
...
@Suppress("MaxLineLength")
inline fun <reified T : Any> AppQualityDomainObjectContainer<T>.getCommon(defaultName: String = COMMON_CONTAINER_NAME): T? {
    return findByName(defaultName)
}
